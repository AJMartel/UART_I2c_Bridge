...
void Uart2I2CBridge::readSendBytes(const String &in, long int  sendByteCount, uint8_t& newPos, uint8_t& commaOffset){
  ...
  for(; i < sendByteCount; ++i){
    ...
    if (i > 0) commaOffset = 1;
    String sendByte = in.substring(posSendByteBegin+i*kHexLength+commaOffset,posSendByteBegin+kHexLength+i*kHexLength+commaOffset);  
...
// Format: 0x<HEX Address of Destination>:<Number of send Bytes>:0x<Send Byte0>[,0x<Send Byte1>[,...]]:<Number of expected bytes>
...
// Example2 : 0x40:2:0x12,0xEF:1
  ...
  expectedByteCount_ = str.substring(posExpectedByteCount,posExpectedByteCount+1);  
